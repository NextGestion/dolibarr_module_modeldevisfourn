 ...
 */
class pdf_devisfourn extends ModelePDFSupplierProposal
{
	/**
			...
			if (file_exists($dir)) {
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					*/
					...
					$posxqty = $this->page_largeur - $this->posxqty - $this->marge_droite;
					if (!empty($conf->global->PRODUCT_USE_UNITS)) {
						$posxqty = $this->posxunit - $this->posxqty;
					}

					// Quantity
					$qty = pdf_getlineqty($object, $i, $outputlangs, $hidedetails);
					$pdf->SetXY($this->posxqty, $curY);
					$pdf->MultiCell($posxqty - 0.8, 4, $qty, 0, 'C'); // Enough for 6 chars
